...
<body>
	<form action="/payserch">
		<header
			class="navbar navbar-dark sticky-top bg-dark flex-md-nowrap p-0 shadow">
			<a class="navbar-brand col-md-3 col-lg-3 me-0 px-3" href="#">INSURANCE
				MANAGEMENT </a>
			...
			<input class="form-control form-control-dark w-50" type="text"
				placeholder="Search" aria-label="Search" name="policyno">
				...
				<table
					...
					<thead>
						<tr>
							...
							<th scope="col">Policy Interest</th>
							<th scope="col">Payment Date</th>
							<th scope="col">Payment Installment</th>
							<th scope="col">Due Maturity Year</th>
							<th scope="col">Due Policy Amount</th>
							<th scope="col">Claims With Interest</th>			
						</tr>
					</thead>
					<tbody>
						<c:forEach var="s" items="${payments}">
							<tr>
								<td>${s.slno}</td>
								<td>${s.policyno}</td>
								<td>${s.policyholdername}</td>
								<td>${s.policyname}</td>
								<td>${s.policyamount}</td>
								<td>${s.maturityyear}</td>
								<td>${s.policyinterest}</td>
								<td>${s.paymentdate}</td>
								<td>${s.paymentinstallment}</td>
								<td>${s.duematurityyear}</td>
								<td>${s.duetotalpolicyamount}</td>
								<td>${s.claimswithinterest}</td>
							</tr>
						</c:forEach>
					</tbody>
				</table>
