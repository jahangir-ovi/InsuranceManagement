	...
	<form action="/search">
		<header
			...
			<input class="form-control form-control-dark w-50" type="text"
				placeholder="Search" aria-label="Search" name="policyholdername">
	...
	<div class="container-fluid">
		<div class="row">
			...
			<main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
				...
				<h1>All Policy Holder :</h1>
				<table
					class="table table-striped container mt-3 shadow border:1px solid-black">
					<thead>
						<tr>
							<th scope="col">Policy No</th>
							<th scope="col">Policy Holder Pic</th>
							<th scope="col">Policy Name</th>
							<th scope="col">Policy Accepted Date</th>
							<th scope="col">Policy Holder Name</th>
							<th scope="col">Policy Holder Gender</th>
							<th scope="col">Policy Holder Age</th>
					...
					<tbody>
						<c:forEach var="s" items="${st}">
							<tr>
								<td>${s.policyno}</td>
								<td>${s.policyholderpic}</td>
								<td>${s.policyname}</td>
								<td>${s.policyacceptdate}</td>
								<td>${s.policyholdername}</td>
								<td>${s.policyholdergender}</td>
								<td>${s.policyholderage} Years</td>
								<td>${s.policyamount} taka</td>
								<td>${s.maturityyear} Years</td>
								<td>${s.policyinterest} %</td>
								<td>${s.holdermonthlyincome} taka</td>
								<td>${s.maritalstatus}</td>
								<td>${s.nomineename}</td>
								<td>${s.nomineeage} Years</td>
								<td>${s.nomineerelation}</td>
								<td>${s.nomineepic}</td>
								<td>${s.healthissue}</td>

								<td>
									<form action="/updateform/${s.getPolicyno()}">
								...
								<td>
									<form action="/deleteholder/${s.getPolicyno()}">
